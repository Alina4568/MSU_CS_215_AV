...
Node * search_push(Node * list, Data a, Data b);
...
Node * push_front(Node * list, Data d) // добавляет элемент в начало (динамическфя память)
{
...
Node * push_back(Node * list, Data d) // добавляет элемент в конец (динамическфя память)
{
...
Node * search_push(Node * list, Data a, Data b) // добавление элемента после (а) если он есть, или добавление в конец
{
    int flag = 0;
    Node * p;
    p = list->next;
...
void test_alloc()  //создание с динамической памятью
{
    Node z;
    Node * list = &z;
...
void test() // создание без динамической памяти
{
    Node z, a, b, c, u, w;
    Node * list = &z;
    z.data = 0;
    ...
    print(list);        // 3 17 21
    print_back(list);   // 21 17 3 печать в обратном направлении
